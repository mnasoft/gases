...
(defclass molecule ()
  ((molecule-name-ru       :accessor molecule-name-ru       :initarg :molecule-name-ru       :initform "" :documentation "Обозначение русскоязычное")
   ...
   (molecule-μcp-a-b-c     :accessor molecule-μcp-a-b-c     :initarg :molecule-μcp-a-b-c     :initform "" :documentation "Коэффициенты для расчета мольной теплоемкости ккал/(моль*К). Данные взяты из файла ./doc/111.jpg (см. мультитехнический справочник Интернет).")
  ...
  (:documentation "Представляет молекулу вецества."))
...
(defclass sp ()
  ...
  (:documentation "Представляет молекулу вещества (Species name/Formula). Данные для элементов взяты из базы данных NASA (см. https://www.grc.nasa.gov/www/CEAWeb/)"))
...
(defclass composition ()
  ((composition-components :accessor composition-components :initarg :composition-components :initform nil :documentation "Содержит список компонентов."))
    (:documentation "Представляет смесь, состоящую из объектов класса component."))
